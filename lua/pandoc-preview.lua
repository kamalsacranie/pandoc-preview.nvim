...
M.preview = function()
    ...
    local latex_template_path = tempdir ..
        "/standalone.latex" -- check what tex packages are required. maybe i can just put them in the repo
        ...
        vim.defer_fn(function()
            vim.system({
                ...
                vim.defer_fn(function()
                    ...
                    local curr_win_w = vim.api.nvim_win_get_width(vim.api
                        .nvim_get_current_win())
                    local float_width = curr_win_w / 3
                    local float_cols = math.ceil(float_width)

                    local img_scalar = (math.floor(float_width) / image.image_width)

                    local float_rows = math.floor(image.image_height * img_scalar /
                        2)
                    ...
                    image:render { width = math.floor(float_width) - 2, x = vim.api.nvim_win_get_width(vim.api
                        .nvim_get_current_win()) - math.floor(float_width), y = 1 }
                    local ns = vim.api.nvim_create_namespace "pandoc-preview"
                    vim.on_key(
                        function(key) -- not sure how to stop this function running
                            if
